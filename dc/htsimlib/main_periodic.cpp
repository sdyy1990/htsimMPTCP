...
// Simulation params
simtime_picosec RTT1=timeFromMs(25);
simtime_picosec RTT2=timeFromMs(50);
double targetwnd = 30;
int NUMFLOWS = 2;

//path 3 has periodic flows, eq. to 1 tcp
#define TCP_1 3
#define PERIOD_ACTIVE 10
#define PERIOD_IDLE 11
//#define PERIOD_IDLE 0

#define TCP_2 1

linkspeed_bps SERVICE1 = speedFromPktps(2000);//NUMFLOWS * targetwnd/timeAsSec(RTT1));
linkspeed_bps SERVICE2 = speedFromPktps(1000);//NUMFLOWS * targetwnd/timeAsSec(RTT2));

#define RANDOM_BUFFER 3

#define FEEDER_BUFFER 2000

mem_b BUFFER1=memFromPkt(RANDOM_BUFFER+50);//NUMFLOWS * targetwnd);
...
int main(int argc, char **argv) {
    ...
    //TCP flows on path 1
    TcpSrcPeriodic* tcpSrc;
    TcpSinkPeriodic* tcpSnk;
    route_t* routeout;
    route_t* routein;
    double extrastarttime;

    for (int i=0; i<TCP_1; i++) {
        tcpSrc = new TcpSrcPeriodic(NULL,NULL,eventlist,timeFromSec(PERIOD_ACTIVE),timeFromSec(PERIOD_IDLE));
        tcpSrc->setName("Tcp1"+ntoa(i));
        logfile.writeName(*tcpSrc);
        tcpSnk = new TcpSinkPeriodic();
        tcpSnk->setName("tcp1"+ntoa(i));
    ...
    for (int i=0; i<TCP_2; i++) {
        tcpSrc = new TcpSrcPeriodic(NULL,NULL,eventlist,timeFromSec(0),timeFromSec(0));
        tcpSrc->setName("Tcp2"+ntoa(i));
        logfile.writeName(*tcpSrc);
        tcpSnk = new TcpSinkPeriodic();
        tcpSnk->setName("tcp2"+ntoa(i));
        logfile.writeName(*tcpSnk);
    ...
    //MTCP flow 1
    tcpSrc = new TcpSrcPeriodic(NULL,NULL,eventlist,0,0);
