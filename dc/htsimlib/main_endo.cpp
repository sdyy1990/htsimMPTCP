...
linkspeed_bps SERVICE1 = speedFromPktps(100000);//NUMFLOWS * targetwnd/timeAsSec(RTT1));
linkspeed_bps SERVICE2 = speedFromPktps(60000);//NUMFLOWS * targetwnd/timeAsSec(RTT2));

#define RANDOM_BUFFER 3

#define FEEDER_BUFFER 2000

mem_b BUFFER1=memFromPkt(RANDOM_BUFFER+5000);//NUMFLOWS * targetwnd);
mem_b BUFFER2=memFromPkt(RANDOM_BUFFER+15000);//NUMFLOWS * targetwnd);

void exit_error(char* progr) {
    cout << "Usage " << progr << " [UNCOUPLED(DEFAULT)|COUPLED_INC|FULLY_COUPLED|COUPLED_EPSILON]" << endl;
    exit(1);
}
...
int main(int argc, char **argv) {
    ...
    Queue pqueue2(SERVICE2, memFromPkt(FEEDER_BUFFER), eventlist,NULL);
    ...
    TcpRtxTimerScanner tcpRtxScanner(timeFromMs(100), eventlist);
    ...
    for (int i=0; i<TCP_1; i++) {
        ...
        tcpSrc->setName("stcp1_"+ntoa(i));
        logfile.writeName(*tcpSrc);
        tcpSnk = new TcpSink();
        tcpSnk->setName("Tcp1_"+ntoa(i));
    ...
    for (int i=0; i<TCP_2; i++) {
        tcpSrc = new TcpSrc(NULL,NULL,eventlist);
        tcpSrc->setName("stcp2_"+ntoa(i));
        logfile.writeName(*tcpSrc);
        tcpSnk = new TcpSink();
        tcpSnk->setName("Tcp2_"+ntoa(i));
        logfile.writeName(*tcpSnk);
    ...
    rtt = timeAsSec(RTT2);
    logfile.write("# rtt="+ntoa(rtt));
    logfile.write("# num_tcp_flows_bottleneck1="+ntoa(TCP_1));
    logfile.write("# num_tcp_flows_bottleneck2="+ntoa(TCP_2));
    logfile.write("# num_multipath_flows=1");
