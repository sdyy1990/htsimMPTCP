...
// Simulation params
simtime_picosec RTT1=timeFromMs(20);
simtime_picosec RTT2=timeFromMs(200);
double targetwnd = 30;
int NUMFLOWS = 2;
...
linkspeed_bps SERVICE1 = speedFromPktps(500);//NUMFLOWS * targetwnd/timeAsSec(RTT1));
...
mem_b BUFFER1=memFromPkt(RANDOM_BUFFER+10);//NUMFLOWS * targetwnd);
mem_b BUFFER2=memFromPkt(RANDOM_BUFFER+20);//NUMFLOWS * targetwnd);
...
int main(int argc, char **argv) {
    ...
    for (int i=0; i<TCP_1; i++) {
        cbrSrc = new CbrSrc(eventlist,speedFromPktps(490),timeFromSec(0.5),timeFromSec(2));
        cbrSrc->setName("Cbr1");
    ...
    for (int i=0; i<TCP_2; i++) {
        cbrSrc = new CbrSrc(eventlist,speedFromPktps(10));
        cbrSrc->setName("Cbr2");
        logfile.writeName(*cbrSrc);
        cbrSnk = new CbrSink();
        cbrSnk->setName("Cbr2");
