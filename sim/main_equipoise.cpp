...
#define TCP_1 4
#define PERIOD_ACTIVE 1
#define PERIOD_IDLE 10
...
mem_b BUFFER2=memFromPkt(RANDOM_BUFFER+100);//NUMFLOWS * targetwnd);
...
int main(int argc, char **argv) {
    ...
    TcpSrc* tcpSrc;
    TcpSink* tcpSnk;
    TcpSrcPeriodic* ptcpSrc;
    TcpSinkPeriodic* ptcpSnk;
    route_t* routeout;
    route_t* routein;
    double extrastarttime;

    for (int i=0; i<TCP_1; i++) {
        ptcpSrc = new TcpSrcPeriodic(NULL,NULL,eventlist,timeFromSec(PERIOD_ACTIVE),timeFromSec(PERIOD_IDLE));
        ptcpSrc->setName("Tcp1"+ntoa(i));
        logfile.writeName(*ptcpSrc);
        ptcpSnk = new TcpSinkPeriodic();
        ptcpSnk->setName("tcp1"+ntoa(i));
        logfile.writeName(*ptcpSnk);
        tcpRtxScanner.registerTcp(*ptcpSrc);
        ...
        routeout->push_back(ptcpSnk);

        routein  = new route_t();
        routein->push_back(ptcpSrc);

        extrastarttime = drand()*50;
        ptcpSrc->connect(*routeout,*routein,*ptcpSnk,timeFromMs(extrastarttime));
        sinkLogger.monitorSink(ptcpSnk);
    }

    //TCP flow on path 2
    for (int i=0; i<TCP_2; i++) {
        ptcpSrc = new TcpSrcPeriodic(NULL,NULL,eventlist,timeFromSec(PERIOD_ACTIVE),timeFromSec(PERIOD_IDLE));
        ptcpSrc->setName("Tcp2"+ntoa(i));
        logfile.writeName(*ptcpSrc);
        ptcpSnk = new TcpSinkPeriodic();
        ptcpSnk->setName("tcp2"+ntoa(i));
        logfile.writeName(*ptcpSnk);
